    ...
    <div className="min-h-screen flex flex-col items-center justify-center bg-gray-900 text-gray-100 p-8">
      <div className="w-full flex items-center mb-6">
        ...
        <div className="flex-1 flex justify-center">
          <h1 className="text-2xl sm:text-4xl font-bold text-blue-300">How to prepare for future</h1>
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-4">
          {points.map((p, idx) => (
            <div
              ...
              {idx < revealedCount ? (
                <>
                  <h3 className="font-semibold text-blue-200">{p.direction}</h3>
                  <h4 className="text-blue-300">Skills : {p.skills}</h4>
                  <span>{p.example}</span>
                </>
              ) : (
